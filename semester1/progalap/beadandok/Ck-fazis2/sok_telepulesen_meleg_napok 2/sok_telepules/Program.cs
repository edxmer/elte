...
namespace sok_telepules {
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[,] h;
            // Deklaráció: kimenet
            // Statikus tömbbel dolgozok, így szükség van a darabszámra is,
            // amit t-vel fogok jelölni
            int t;
            int[] ts;
        ...
        static (int, int[]) kivalogat(int[,] h) {
            int[] ts;
            int t, i;
            
            t = 0;
            ts = new int[h.GetLength(1)];

            for (i = 0; i < h.GetLength(1); ++i) {
                if (legalabbfelen30(h, i)) {
                    ts[t] = i;
                    ++t;
                }
            }
